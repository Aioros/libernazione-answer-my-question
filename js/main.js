			...
			this.construct = function(settings) {
				...
				return this.each(function() {	
					...
					$(config.cssFirst,pager).click(function() {
						...
						$("#amq_list").find('tr[data-status!="0"]').draggable(draggableOpts);
						return false;
					});
...
jQuery(function($) {
	...
	$("body").on("click", ".delete", function(){
		...
		jConfirm('Are you sure you want to delete this question?', 'Delete Question', function(result) {
			if(result == true){
			$.post($("#plugin_path").val()+'/delete_question.php', {"ids": qID}, function(obj){
				if(obj.result == true){
	...
	$("body").on("click", ".post", function(){
		var qID = $(this).attr('rel');
		var parentRow = $(this).parent().parent();
		jConfirm('Are you sure you want to post this question?', 'Post Question', function(result) {
			if(result == true){
				$.post($("#plugin_path").val()+'/post_question.php', {"ids": qID}, function(obj){
					if(obj.result == true){
	...
	$("#amq_modal").dialog({
		...
		width: 700,
		close: function() {
			$("#amq_modal").dialog("close");
			$("#amq_modal input[name='id']").val("");
			$("#amq_modal input[name='notify']").val("");
			$("#amq_modal input[name='user_email']").val("");
			$("#question_title em").html("");
			$("#subject").val("");
			$("#question").val("");
			$("#answer_title").html("");
			$("#question_answered").remove();
			$("#email").remove();
	...
	$("body").on("click", ".answer", function(){
		...
		$.getJSON($("#plugin_path").val()+"getQuestionInfo.php", {id: thisID}, function(data) {
			$("#amq_modal input[name='id']").val(data.id);
			$("#amq_modal input[name='notify']").val(data.notify_user == 1 ? 1 : 0);
			$("#amq_modal input[name='user_email']").val(data.user_email);
			$("#question_title em").html(data.user_name);
			...
			if (data.answer.length > 0) {
				...
				$('<a href="mailto:' + data.user_email + '" id="email">Answer this user directly &raquo;</a>').insertBefore($("#save"));
			}
			$("#amq_modal").dialog("option", "title", "Answer");
			$("#amq_modal").dialog("open");
		});
		return false;
	});
	...
	// Image settings (media upload / select)
	// Uploading files
	var file_frame;

	$('.amq_image_select').live('click', function( event ){
		...
		file_frame.on('select', function() {
			...
			$("#amq_imageid").val(attachment.id);
			$("#amq_imageurl").val(attachment.url);
			$("#amq_image").attr("src", attachment.url);
			$("#amq_image").show();
		});
	...
	// Post answers
	$("body").on("click", ".to_be_posted .remove", function(){
		var toBePosted = $(this).closest(".to_be_posted");
		$(this).parent().remove();
		if (toBePosted.find('tr').length == 0)
			toBePosted.find(".post_answers").prop("disabled", true);
	});
	
	draggableOpts = {
		cursor: "move",
		helper: function(event) {
			var row = $(event.target).closest('tr').clone();
			row.find(".actions,.display_status,.published,.answered,.unanswered,.select-delete").remove();
			return $('<div class="drag-answer"><table></table></div>').find('table').addClass("tablesorter").append(row).end().appendTo('body');
		},
		cursorAt: {top: 100, left: 250}
	};
	droppableOpts = {
		drop: function(event, ui) {
			if (ui.helper.hasClass("ui-draggable-dragging")) {
				if ($(this).find(".to_be_posted tr").length == 0) {
					$(this).find(".to_be_posted").show();
				}
				$(this).find('.to_be_posted').append(ui.helper.find("tr").clone().append($("<td>").addClass("remove")));
				$(this).find('.to_be_posted').remove(".actions");
				$(this).find(".post_answers").prop("disabled", false);
			}
		}
	};
	
	$(".to_be_posted").sortable({items: " tr"});
	
	$(".amq_post").click(function() {
		$("#post_creation").hide();
		$("#post_editing .post_answers").removeAttr("disabled").attr("data-post-id", $(this).attr("data-post-id"));
		$("#post_editing").show();
		$("#post_editing .select_answers").click();
		$("#post_editing").droppable(droppableOpts);
		$("#post_editing .to_be_posted").empty().show();
		$(this).find("tbody tr").each(function() {
			var tr = $("<tr>")
				.attr("data-id", $(this).attr("data-question-id"))
				.addClass("ui-draggable")
				.append($(this).find("td").clone())
				.append($("<td>").addClass("remove"));
			$("#post_editing .to_be_posted").append(tr);
		});
	});
	
	$(".select_answers").click(function() {
		var parentDiv = $(this).closest("div");
		if (parentDiv.attr("id") == "post_creation")
			$("#post_editing").hide();
		else
			$("#post_creation").hide();
		parentDiv.css("width", "70%");
		parentDiv.find(".drag_label").show();
		$("#amq_list").find('tr[data-status!="0"]').draggable(draggableOpts);
		$("#amq_list").find('tr[data-status!="0"]').draggable("enable");
		parentDiv.droppable(droppableOpts);
		
	
		parentDiv.find(".cancel_answers").show();
		parentDiv.find(".select_answers").hide();
		parentDiv.find(".select_answers").prop("disabled", true);
	});
	
	$(".cancel_answers").click(function() {
		var parentDiv = $(this).closest("div");
		$("#post_editing").hide();
		$("#post_creation").show();
		parentDiv.css("width", "auto");
		$("#amq_list").find('tr[data-status!="0"]').draggable("destroy");
		parentDiv.find(".cancel_answers").hide();
		parentDiv.find(".select_answers").prop("disabled", false);
		parentDiv.find(".select_answers").show();
		parentDiv.find(".post_answers").prop("disabled", true);
		parentDiv.find(".to_be_posted").empty().hide();
		parentDiv.find(".drag_label").hide();
	});
	
	$(".post_answers").click(function() {
		var parentDiv = $(this).closest("div");
		var qIDs = [];
		parentDiv.find(".to_be_posted tr").each(function(index) {
			qIDs.push($(this).data("id"));
		});
		qIDs = qIDs.join();
		var confirmText = 'Are you sure you want to create a post with these questions?';
		var postId = -1;
		if ($(this).attr("data-post-id")) {
			confirmText = 'Are you sure you want to update the post with these questions?';
			postId = $(this).attr("data-post-id");
		}
		jConfirm(confirmText, 'Post Questions', function(result) {
			if (result == true){
				$("#sending-loader").fadeIn('fast', function() {
					//Bring the loader into the viewport
					var viewportWidth = $(window).width(),
					viewportHeight = $(window).height(),
					$foo = $('#sending-loader'),
					elWidth = $foo.width(),
					elHeight = $foo.height();
					$foo.offset({top: viewportHeight/2 - elHeight/2, left: viewportWidth/2 - elWidth/2});
					elOffset = $foo.offset();
					$(window)
						.scrollTop(elOffset.top + (elHeight/2) - (viewportHeight/2))
						.scrollLeft(elOffset.left + (elWidth/2) - (viewportWidth/2));
				});
				$.post($("#plugin_path").val()+'/post_question.php', {"ids": qIDs, "postId": postId}, function(obj) {
	...
	// Delete answers
	$("body").on("click", "#amq_list .select-delete input", function(){
		$("#delete_answers").prop("disabled", $("#amq_list .select-delete input:checked").length == 0);
	});
	...
	$("#select_delete").click(function() {
		...
		$(".select-delete").show();
		var allRows = $("#amq_list").get(0).config.rowsCopy;
		for (r in allRows) {
			$(allRows[r]).children(".select-delete").show();
		}
	});
	...
	$("#cancel_delete").click(function() {
		...
		$("#select_delete").show();
		$("#delete_answers").prop("disabled", true);
		$(".select-delete").hide();
		var allRows = $("#amq_list").get(0).config.rowsCopy;
		for (r in allRows) {
			$(allRows[r]).find(".select-delete input:checked").prop("checked", false);
			$(allRows[r]).children(".select-delete").hide();
		}		
	});
	
	$("#delete_answers").click(function() {
		var qIDs = [];
		var allRows = $("#amq_list").get(0).config.rowsCopy;
		for (r in allRows) {
			var checkbox = $(allRows[r]).find(".select-delete input").get(0);
			if (checkbox.checked)
				qIDs.push($(checkbox).attr('rel'));
		}
		qIDs = qIDs.join();
		jConfirm('Are you sure you want to delete these questions?', 'Delete Questions', function(result) {
			if (result == true){
				$("#sending-loader").fadeIn('fast', function() {
					//Bring the loader into the viewport
					var viewportWidth = $(window).width(),
				...
				$.post($("#plugin_path").val()+'/delete_question.php', {"ids": qIDs}, function(obj) {
					$("#sending-loader").fadeOut('fast');
					if (obj.result == true) {
						document.location.reload(true);
					} else {
